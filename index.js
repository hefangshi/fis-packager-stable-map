...
module.exports = function (ret, conf, settings, opt) {
    ...
    var hit = function (subpath, regs, file) {
    ...
    function sortPkg(pkg) {
        ...
        var pkgFileMap = {};
        var checked = {};
        var ids = pkg.map(function (file) {
            var id = file.getId();
            pkgFileMap[id] = file;
            return id;
        });
        stable.inplace(ids);
        ids.forEach(checkRequire);
        ...
        function checkRequire(id) {
            ...
            var file = ret.map.res[id];
            file && file.deps && file.deps.forEach(function (depId) {
                checkRequire(depId);
            });
            if (pkgFileMap[id]) {
                sortedPkg.push(pkgFileMap[id]);
            }
        }
        return sortedPkg;
    }
    ...
    fis.util.map(pkgMap, function (pid, pkg) {
        ...
        pkg.pkgs.forEach(function (pkg) {
            var sortedPkg = sortPkg(pkg);
            sortedPkg.forEach(function (file) {
                var id = file.getId();
                if (ret.map.res[id]) {
